/*
// Compute posteriro likelihoods of mixture MDP models, assuming that initial
// values are fixed.
// Author: Chiyoung Ahn
*/
...
// [[Rcpp::export]]
SEXP PosteriorMDPInitialFixed (Rcpp::NumericMatrix y_rcpp,
					...
					Rcpp::NumericVector gamma_rcpp)
{
  ...
  double likelihood = 0;
	...
	for (int i = 0; i < N; i++)
	{
		...
		// compute posteriors
		for (int j = 0; j < M; j++)
			posteriors(i, j) = likelihoods_i[j] / likelihood_i;
